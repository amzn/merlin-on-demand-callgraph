...
public abstract class MerlinSolver extends SyncPDSSolver<NodeState, Value, Property, Weight.NoWeight> {
    ...
    public Query initialQueryWithDirection() {
        return new Query(initialQuery, isForward());
    }
    ...
    public static BufferedImage visualizeCallPDS(WeightedPushdownSystem<NodeState, INode<Value>, Weight.NoWeight> callPDS) {
        ...
        final Graph<Value, CallEdge> graph = // new DirectedMultigraph<>(CallEdge.class);
                GraphTypeBuilder.<Value, CallEdge>directed().allowingMultipleEdges(true).allowingSelfLoops(true)
                        .buildGraph();
        final Consumer<Value> ensureVertexExists = (vertex) -> {
        ...
        for (final var rule: callPDS.getAllRules()) {
            ...
            final CallEdge edge = new CallEdge(rule);
            graph.addEdge(src, dst, edge);
        }
        final var dotExporter = new DOTExporter<Value, CallEdge>();
...
//    public abstract AbsReturnSite possibleReturnSites();

    private static class CallEdge {
        private final Rule<NodeState, INode<Value>, Weight.NoWeight> rule;

        public CallEdge(Rule<NodeState, INode<Value>, Weight.NoWeight> rule) {
            this.rule = rule;
        }
        ...
        public String toString() {
            if (rule instanceof PushRule pushRule) {
                return pushRule.getL1() + " ~> " + pushRule.getL2() + " :: " + pushRule.getCallSite();
            } else if (rule instanceof PopRule popRule) {
                return popRule.getL1() + " ~> " + "[]";
            } else if (rule instanceof NormalRule) {
                return rule.getL1() + " ~> " + rule.getL2();
            }
    ...
    public final boolean addDataFlowsForUnresolvedFunctionCalls() {
        ...
        for (final var state : this.getReachedStates()) {
            if (state.stmt().getNode() instanceof CallNode callNode && !FlowgraphUtils.isTAJSInternal(callNode)) {
                final var callees = AbstractFlowFunctions.resolveFunctionCall(callNode, queryManager).toJavaSet();
                final var theCall = callNode;
    ...
    // TODO: refactor into a more high-level API
    public WeightedPAutomaton<NodeState, INode<Value>, Weight.NoWeight> getCallAutomaton() {
        return callAutomaton;
    }

    public boolean areCallsUnbalanced() {
        Set<Rule<NodeState, INode<Value>, Weight.NoWeight>> x= this.callingPDS.getAllRules();
//        this.fieldPDS
        WeightedPushdownSystem<NodeState, INode<Value>, Weight.NoWeight> callingPDS = this.callingPDS;
        return callAutomaton.isUnbalancedState(new SingleNode<>(initialQuery.fact()));
    }

    public WeightedPushdownSystem<NodeState, INode<Value>, Weight.NoWeight> getCallingPDS() {
